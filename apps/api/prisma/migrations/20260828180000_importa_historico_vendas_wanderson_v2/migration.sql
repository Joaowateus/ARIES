...
-- wanderson). A tentativa anterior (20260828170000_reimporta_historico_
-- vendas_wanderson) foi revertida por precaução durante uma instabilidade
-- em produção não relacionada a ela (confirmado depois: o time de vendas
-- verificou que voltou tudo ao normal, e o histórico do Wanderson
-- continuava ausente — ou seja, essa reimportação nunca chegou a rodar).
-- Reaplicando agora.
--
...
-- existir. Cada bloco só insere se a combinação (nomeCliente + criadaEm)
-- ainda não existir, então é seguro mesmo se a importação original já
-- tiver funcionado parcialmente.

-- Importa o histórico real de vendas do Wanderson (jan-jul/2026),
-- extraído do relatório de balanceamento comercial (BALANCEAMENTO.pdf).
...
-- tem a jornada do lead (só mês do fechamento), e inventar etapas
-- intermediárias distorceria a conversão real do Funil de Vendas e do
-- funil individual. Sem nome de cliente na fonte: usa um rótulo claro de
-- venda histórica em vez de inventar uma pessoa.
--
-- É condicional (CTE "v" busca o usuário pelo e-mail real do Wanderson) e
-- vira no-op silencioso em qualquer banco onde essa conta não existir —
-- só popula de fato em produção.
